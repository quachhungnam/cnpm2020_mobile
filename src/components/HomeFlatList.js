...
import {get_all_province, get_one_province} from '../api/province_api';
import {
  get_district_with_province,
  get_one_district,
} from '../api/district_api';
import {get_all_posttypes, get_one_posttype} from '../api/posttype_api';
...
export default class HomeFlatList extends Component {
  ...
  componentDidMount() {
    this.refreshDataFromServer(
      this.state.seleted_province,
      this.state.selected_district,
      this.state.selected_posttype,
    );
  ...
  getPostsFromServerWithPage() {
    getPostsFromServerWithPage(this.state.pageNumber)
      .then(res => {
        this.setState({
          postsFromServer: this.state.postsFromServer.concat(
            res.post.filter(
  ...
  change_district = value => {
    ...
    if (value !== -1) {
      ...
      return this.refreshDataFromServer(
        this.state.seleted_province,
        value,
        this.state.selected_posttype,
      );
  ...
  change_posttypes = posttype_id => {
    ...
    if (posttype_id === -1) {
      ...
      if (
        this.state.selected_district === -1 &&
        this.state.seleted_province !== -1
      ) {
      ...
      this.setState({refreshing: true});
      getPostsFromServerByType(posttype_id)
        .then(res => {
          if (this.state.seleted_province === -1) {
            this.setState({
          ...
          if (
            ...
            this.setState({
              postsFromServer: res
                .filter(
                  item =>
                    item.status_id.code === 1 || item.status_id.code === 2,
                )
                .filter(
                  item =>
                    item.status_id.code === 1 || item.status_id.code === 2,
                ...
                .filter(
                  item => item.province_id.code === this.state.seleted_province,
                ),
            });
            return this.setState({refreshing: false});
          }
          this.setState({
            postsFromServer: res
              ...
              .filter(
                item => item.district_id.code === this.state.selected_district,
              ),
          });
          this.setState({refreshing: false});
          // this.get_district()
  ...
  render() {
    let tieuchi;
    if (
      this.state.nameSelectedPosttype === -1 &&
      this.state.nameSelectedDistrict === -1 &&
      this.nameSelectedProvince === -1
    ) {
      tieuchi = (
        <Text style={{flex: 1, marginLeft: 10, marginTop: 5, marginBottom: 5}}>
          Tất cả bài đăng
        </Text>
      );
    }
    if (
      this.state.nameSelectedPosttype !== -1 &&
      this.state.nameSelectedDistrict !== -1 &&
      this.state.nameSelectedProvince !== -1
    ) {
      tieuchi = (
        <Text style={{flex: 1, marginLeft: 10, marginTop: 5, marginBottom: 5}}>
          {this.state.nameSelectedPosttype} ở {this.state.nameSelectedDistrict},{' '}
          {this.state.nameSelectedProvince}
        </Text>
      );
    }
    if (
    ...
    if (
      ...
      tieuchi = (
        <Text style={{flex: 1, marginLeft: 10, marginTop: 5, marginBottom: 5}}>
          Tất cả bài đăng {this.state.nameSelectedDistrict},{' '}
          {this.state.nameSelectedProvince}
        </Text>
      );
    }
        ...
        <View
          ...
          <View style={{flex: 1, flexDirection: 'row'}}>
            ...
            <Picker
              ...
              onValueChange={(itemValue, itemIndex) => {
                this.change_province(itemValue);
              }}>
              {/* Hien thi cac tinh thanh */}
              <Picker.Item label="Tỉnh/thành phố" value={-1} key={-1} />
            ...
            <Picker
              ...
              onValueChange={(itemValue, itemIndex) => {
                this.change_district(itemValue);
              }}>
              {/* Hien thi toan bo quan, huyen */}
              <Picker.Item label="Quận/Huyện" value={-1} key={-1} />
          ...
          <View style={{flex: 1}}>
            ...
            <Picker
              ...
              onValueChange={(itemValue, itemIndex) => {
                this.change_posttypes(itemValue);
              }}>
              <Picker.Item label="Loại tin" value={-1} key={-1} />
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (
                this.state.selected_posttype === -1 ||
                this.seleted_province === -1
              ) {
