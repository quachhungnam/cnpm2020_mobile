...
export default function ListPost(props) {
    ...
    const getAllPosts = async () => {
        try {
            ...
            if (res.error) {
                ...
                const post_filter = res.post.filter(item => item.status_id.code === 1
                    || item.status_id.code === 2)
    ...
    const filter_post = (condition) => {
        ...
        const post_filter = all_post.filter((post) =>
            (
                (condition.province_code != -1 ? post.province_id.code == condition.province_code : 1)
                && (condition.district_code != -1 ? post.district_id.code == condition.district_code : 1)
                && (condition.post_type_id != -1 ? post.post_type_id._id == condition.post_type_id : 1)
            ))
        set_arr_post(post_filter)
    }
